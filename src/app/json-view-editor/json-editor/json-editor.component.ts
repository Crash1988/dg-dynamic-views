...
// import JSONEditor from 'jsoneditor';
import { IError, JsonEditorMode, JsonEditorOptions, JsonEditorTreeNode } from './json-editor-options';
...
export class JsonEditorComponent implements ControlValueAccessor, OnInit, OnDestroy {
  ...
  public id = 'dgjsoneditor' + Math.floor(Math.random() * 1000000);
  private _data: Object = {};
  private editor!: InstanceType<typeof JSONEditor>;
  ...
  ngOnInit() {
    ...
    if (
      optionsCopy.mode === 'text' ||
      optionsCopy.mode === 'code' ||
      (optionsCopy.modes && (optionsCopy.modes.indexOf('text') !== -1 || optionsCopy.modes.indexOf('code') !== -1))
    ) {
